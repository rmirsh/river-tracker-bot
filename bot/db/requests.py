...
async def is_first_time(user_id: int) -> bool:
    ...
    This function checks if a user with the given user_id is the first time
    using the bot by querying the database.

    Args:
        user_id (int): The user ID to check subscription for.

    Returns:
        bool: True if the user is the first time using the bot, False otherwise.
    """
